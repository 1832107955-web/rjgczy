...
for name, code in diagrams.items():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # save raw bytes as jpg if jpeg, else as svg if svg
            if 'jpeg' in ctype or 'jpg' in ctype:
                ...
                # save svg and optionally rasterize if pillow can't; for now save svg and report
                svg_path = os.path.join(OUT_DIR, f'{name}.svg')
                with open(svg_path, 'wb') as f:
                    f.write(r.content)
                results[name] = svg_path
                print('Saved SVG (no rasterizer available):', svg_path)
            else:
                # unknown content-type: save as bin
                out = os.path.join(OUT_DIR, f'{name}.bin')
                with open(out, 'wb') as f:
                    f.write(r.content)
                results[name] = f'Unknown content saved as {out} (Content-Type: {ctype})'
                print('Saved unknown content type', ctype)
        else:
